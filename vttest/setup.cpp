...
check_8bit_toggle(void)
{
  ...
  if ((report = skip_csi(report)) != NULL
    ...
    result = TRUE;
  if (LOG_ENABLED) {
    fakeio::_fprintf(log_fp, NOTE_STR "%svalid response from DSR 6\n",
            result ? "" : "no ");
  }
  return result;
}
...
find_levels(void)
{
  ...
  if (LOG_ENABLED) {
    fakeio::_fprintf(log_fp, NOTE_STR "Max Operating Level: %d\n", max_level);
    fakeio::_fprintf(log_fp, NOTE_STR "Cur Operating Level: %d\n", cur_level);
    fakeio::_fprintf(log_fp, NOTE_STR "Derived terminal-id: %d\n", actual_id);
  }
...
toggle_7bit_fsm(MENU_ARGS)
{
  ...
  if (LOG_ENABLED) {
    fakeio::_fprintf(log_fp, NOTE_STR "toggle parser from %s to %s\n",
...
save_level(VTLEVEL *save)
{
  ...
  if (LOG_ENABLED)
    fakeio::_fprintf(log_fp, NOTE_STR "save_level(%d) in=%d, out=%d, fsm=%d\n",
            cur_level,
            input_8bits ? 8 : 7,
            output_8bits ? 8 : 7,
            parse_7bits ? 8 : 7);
...
set_level(int request)
{
  ...
  if (LOG_ENABLED)
    fakeio::_fprintf(log_fp, NOTE_STR "set_level(%d)\n", request);
  ...
  if (LOG_ENABLED)
    fakeio::_fprintf(log_fp, NOTE_STR "...set_level(%d) in=%d, out=%d, fsm=%d\n",
            cur_level,
            input_8bits ? 8 : 7,
            output_8bits ? 8 : 7,
            parse_7bits ? 8 : 7);

...
{
  print_str(old_mode);
  fakeio::_fflush(stdout);   /* ensure terminal is updated */
  if (row > 2)
    cud(row - 2);
  if (col > 1)
    cuf(col - 1);
...
tabstop_ruler(const char *tabsr, int row, int col)
{
  ...
  if ((expect = (char*)malloc((size_t) min_cols + 1)) == NULL)
    no_memory();
...
int
any_RQM(MENU_ARGS, RQM_DATA * table, int tablesize, int private_)
{
  int j, row, Pa, Ps;
  int used = -1;
  char chr;
  ...
  for (j = row = 0; j < tablesize; j++) {
    ...
    do_csi((private_ ? "?%d$p" : "%d$p"), table[j].mode);
    report = get_reply();
    printxx("\n     %4d: %-10s ", table[j].mode, table[j].name);
    if (LOG_ENABLED)
      fakeio::_fprintf(log_fp, NOTE_STR "testing %s\n", table[j].name);
...
any_decrqss2(const char *msg, const char *func, const char *expected)
{
  ...
  if (code == 1) {
    ...
    char *part = (char*)malloc(5 + strlen(report) + strlen(suffix));

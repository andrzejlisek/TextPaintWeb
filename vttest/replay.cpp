...
next_line(void)
{
  ...
  if (rp != NULL) {
    ...
    if (buffer == NULL)
      buffer = (char*)malloc(length = BUF_SIZE);
    buf = buffer;

    while ((tmp = fakeio::_fgets(buf, BUF_SIZE, rp)) != NULL) {
      size_t len = strlen(buf);
      if (len != 0) {
        if (buf[len - 1] == '\n')
          break;
        if ((now + BUF_SIZE) >= length) {
          buffer = (char*)realloc(buffer, length += BUF_SIZE);
        }
        now += len;
        buf = buffer + now;
      } else {
    ...
    if (LOG_ENABLED && debug_level > 1) {
      fakeio::_fprintf(log_fp, "[%04u]: %s", lineno,
...
skip_to_tag(const char *tag)
{
  ...
  while ((check = next_line()) != NULL) {
    if (LOG_ENABLED && debug_level > 0 && strncmp(check, SKIP_STR, need))
      fakeio::_fprintf(log_fp, SKIP_STR "%s", check);
...
pause_replay(void)
{
  ...
  if (LOG_ENABLED) {
    if (paused == 1) {
      fakeio::_fprintf(log_fp, WAIT_STR "%d\n", ++marker);
...
resume_replay(void)
{
  ...
  if (LOG_ENABLED) {
    if (paused == 0) {
      fakeio::_fprintf(log_fp, DONE_STR "%d\n", marker);
      fakeio::_fflush(log_fp);
    }
  }
}

...
void Core2Terminal::EventTick()
{
    ...
    int Processed = CoreAnsi_.get()->AnsiProcess(ProcessTimerStep);
    if (Processed > 0)
    {
...
void Core2Terminal::ConnOpen()
{
    ...
    CoreAnsi_.get()->AnsiProcessReset(true, 0, 1, false);
...
void Core2Terminal::TelnetReport(std::string ReportRequest)
{
    ...
    switch (_(ReportRequest.c_str()))
    {
        ...
        case _("[?53n"): // Locator
            ...
            break;
        case _("[?55n"): // Locator status
            if (SendAnswer)
            {
                SendHex("##_[_?_5_3_n");
            }
            break;
        case _("[?56n"): // Locator type
            if (SendAnswer)
            {
                SendHex("##_[_?_5_7_;_0_n");
            }
            break;
        case _("[?62n"): // DECMSR
            if (SendAnswer)
...
void Core2Terminal::TelnetDisplayInfo(bool NeedRepaint)
{
    ...
    if (WorkStateC == WorkStateCDef::Toolbox)
    {
        ...
        InfoMsg.Add(" Esc Enter - Return to terminal ");
        InfoMsg.Add(" Tab - Move info ");
        InfoMsg.Add(" Backspace - Exit ");
        InfoMsg.Add(" = - Change escape key ");
